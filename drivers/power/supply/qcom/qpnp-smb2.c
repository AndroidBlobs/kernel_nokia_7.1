...
// FIHTDC, IdaChiang, add for debug log {{
...
// FIHTDC, IdaChiang, add for Power Monitor for OTG {{
...
// FIHTDC, IdaChiang, add for FAP usb resistance {{
...
// FIHTDC, IdaChiang, add for H/L temp charging limit reached UI {{
...
//FIHTDC, IdaChiang, add for C1N/B2N sw settings {
...
// FIHTDC, IdaChiang, add for Sharp 1.5A QC
...
static enum power_supply_property smb2_usb_props[] = {
	...
	POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, // FIHTDC, IdaChiang, add for FAT
};
		...
		enum power_supply_property psp)
{
	switch (psp) {
	case POWER_SUPPLY_PROP_CTM_CURRENT_MAX:
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT: // FIHTDC, IdaChiang, add for FAT
		return 1;
	default:
		break;
	}
...
static int smb2_probe(struct platform_device *pdev)
{
	...
	// initiation before batt psy, fihtdc, idachiang
...
// FIHTDC, IdaChiang, add for H/L temp charging limit reached UI
	...
	rc = device_create_file(&pdev->dev, &dev_attr_otg_status); // FIHTDC, IdaChiang, add for Power Monitor for OTG
	rc = device_create_file(&pdev->dev, &dev_attr_type_c_status); // FIHTDC, IdaChiang, add for FAP usb resistance
	rc = device_create_file(&pdev->dev, &dev_attr_lcm_status); // FIHTDC, IdaChiang, add for lcm status
...
// FIHTDC, IdaChiang, add for H/L temp charging limit reached UI
	...
	device_remove_file(&pdev->dev, &dev_attr_otg_status); // FIHTDC, IdaChiang, add for Power Monitor for OTG
	device_remove_file(&pdev->dev, &dev_attr_type_c_status); // FIHTDC, IdaChiang, add for FAP usb resistance
	device_remove_file(&pdev->dev, &dev_attr_lcm_status); // FIHTDC, IdaChiang, add for lcm status
