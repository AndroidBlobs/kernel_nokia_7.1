				...
				struct snd_ctl_elem_value *pUcontrol)
{
	...
	{
		pr_err("TI-SmartPA: %s: Inavlid value %d", __func__, user_data);
		return -1;
	}
	//To exclude None
	if(user_data)
...
static const char *tas2560_calib_test_text[CALIB_COUNT] = {
	"NONE",
	"CALIB_START",
	"CALIB_STOP",
	"TEST_START",
	...
	struct snd_ctl_elem_value *pUcontrol)
{
	...
	if((calib_state >= 0) && (calib_state < CALIB_COUNT))
	{
		pr_info("TI-SmartPA: %s: case %s", __func__, tas2560_calib_test_text[calib_state]);
	}
	else
	{
		pr_err("TI-SmartPA: %s: Inavlid value %d", __func__, calib_state);
		return -1;
	}
	return ret;
}
...
static const char *tas2560_smartamp_enable_text[STATE_COUNT] = {
	"DISABLE",
	"ENABLE"
};
				...
				struct snd_ctl_elem_value *pUcontrol)
{
	...
	if((user_data >= 0) && (user_data < STATE_COUNT))
	{
...
static const char *tas2560_smartamp_bypass_text[STATE_COUNT] = {
	"FALSE",
	"TRUE"
};
...
static const char *tas2560_smartamp_status_text[STATE_COUNT] = {
	"DISABLED",
	"ENABLED"
};
...
static const struct snd_kcontrol_new smartamp_tas2560_mixer_controls[] = {
    ...
    //remove the get status tinymix during power on statge for TAS-3193
